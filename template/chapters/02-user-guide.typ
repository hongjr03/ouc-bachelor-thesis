...
- `appendix` 字段用于引入附录内容，通常我们会把附录内容写在一个单独的 `.typ` 文件里，然后通过 `include` 指令引入进来，这样可以让 `main.typ` 保持简洁。当然也可以像 `acknowledgments` 一样直接输入内容，不过不太建议这么做，因为附录的内容通常比较多，放在一起会显得很乱。如果不需要附录的话，直接把 `appendix` 这一行注释掉或者删掉即可。
